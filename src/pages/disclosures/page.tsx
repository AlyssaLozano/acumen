          ...
          <div className="max-w-4xl mx-auto px-6 space-y-10">
            {sections.map((section, i) => (
              ...
              >
                ...
                {/* Fee breakdown table - only for tuition section */}
                {section.id === 'tuition' && (
                  <div className="mt-8 border border-gold/20 rounded-xl overflow-hidden">
                    <div className="bg-gold/8 px-5 py-3 border-b border-gold/20">
                      <p className="font-mono text-gold text-xs tracking-widest">DIRECT ENROLLMENT - ITEMIZED COST BREAKDOWN</p>
                    </div>
                    <div className="divide-y divide-gold/10">
                      {feeBreakdown.map((row) => (
                        <div key={row.label} className="flex items-center justify-between px-5 py-3">
                          <span className="font-sans text-steel text-sm">{row.label}</span>
                          <span className="font-mono text-ice text-sm font-semibold">{row.amount}</span>
                        </div>
                      ))}
                    ...
                    <div className="bg-navy-950/60 px-5 py-3 border-t border-gold/10">
                      <p className="font-mono text-steel/60 text-xs">
                        Exam voucher fees are NOT included. WIOA-sponsored rate: $8,500 (student out-of-pocket: $0 if approved).
                      </p>
                    </div>
                  </div>
                )}
