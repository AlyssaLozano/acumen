...
export default function AdmissionsForm() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    setSubmitting(true);
    try {
      const ok = await submitToFormspree(form, 'admissions');
      if (ok) setSubmitted(true);
      else alert('Something went wrong. Please email enroll@acumencareeracademy.com.');
    } catch {
      alert('Something went wrong. Please email enroll@acumencareeracademy.com.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          <div className="reveal-element">
            ...
            <p className="font-sans text-steel text-base leading-relaxed mb-4">
            ...
            <p className="font-sans text-steel text-sm leading-relaxed mb-10">
              Both programs are $4,725 total - less than half what national bootcamps charge for programs
              that take twice as long. You graduate with a verifiable portfolio, not just a certificate.
            </p>
            ...
            <div className="mt-8 glass-card border border-gold/15 rounded-2xl overflow-hidden reveal-element">
              ...
              <div className="divide-y divide-gold/10">
                ...
                <div className="px-5 py-4">
                  <p className="font-mono text-gold/70 text-xs tracking-widest mb-3">SECURITY+ READINESS · 240 HRS</p>
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <span className="font-sans text-steel text-sm block">Workforce-Sponsored (WIOA)</span>
                      <span className="font-mono text-steel/40 text-xs">Includes job placement &amp; case management</span>
                    </div>
                ...
                {/* Exam fees */}
                <div className="px-5 py-3 flex justify-between items-center bg-gold/3">
                  <div>
                    <span className="font-sans text-steel text-sm block">Exam Fees (all programs)</span>
                    <span className="font-mono text-steel/40 text-xs">Students are responsible for scheduling &amp; funding exams</span>
                  </div>
                  <span className="font-mono text-red-400 text-sm font-semibold">NOT INCLUDED</span>
                </div>
              </div>
            </div>

            {/* Portfolio deliverables strip */}
            <div className="mt-6 glass-card border border-gold/15 rounded-2xl p-5 reveal-element">
              <p className="font-mono text-gold text-xs tracking-widest mb-4">WHAT YOU&apos;LL GRADUATE WITH</p>
              <div className="grid grid-cols-2 gap-3">
                {[
                  { icon: 'ri-file-text-line', text: 'Written Investigation Reports' },
                  { icon: 'ri-code-s-slash-line', text: '3 Original Detection Rules' },
                  { icon: 'ri-flow-chart', text: 'Multi-Source SOAR Workflow' },
                  { icon: 'ri-archive-drawer-line', text: 'Case Management Evidence' },
                ].map((item) => (
                  <div key={item.text} className="flex items-center gap-2.5">
                    ...
                    <span className="font-sans text-steel text-xs leading-snug">{item.text}</span>
                  </div>
                ))}
              </div>
                ...
                <>
                  ...
                  <div className="flex flex-wrap gap-3 mb-6">
                    <span className="inline-flex items-center gap-1.5 font-mono text-xs text-gold bg-gold/8 border border-gold/20 px-3 py-1.5 rounded-full">
                      <i className="ri-price-tag-3-line text-xs" /> $4,725 total
                    </span>
                    <span className="inline-flex items-center gap-1.5 font-mono text-xs text-ice/70 bg-white/4 border border-white/10 px-3 py-1.5 rounded-full">
                      <i className="ri-time-line text-xs" /> 8 weeks
                    </span>
                    <span className="inline-flex items-center gap-1.5 font-mono text-xs text-ice/70 bg-white/4 border border-white/10 px-3 py-1.5 rounded-full">
                      <i className="ri-folder-chart-line text-xs" /> Portfolio on graduation
                    </span>
                  </div>
